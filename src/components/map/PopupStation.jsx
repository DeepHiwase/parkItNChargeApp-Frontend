...
const PopupStation = ({ popupInfo }) => {
  const { name, description, price, images } = popupInfo
    ...
    <Card sx={{ maxWidth: 400 }}>
      <ImageListItem sx={{ display: 'block' }}>
        <ImageListItemBar
          ...
          title={price === 10 ? 'Normal Charger' : '$' + price}
          position='top'
        />
        <ImageListItemBar
          title={name}
          ...
          {images.map((url) => (
            <SwiperSlide key={url}>
              <Box
                ...
                onClick={() =>
                  dispatch({ type: 'UPDATE_STATION', payload: popupInfo })
                }
              />
            </SwiperSlide>
          ))}
